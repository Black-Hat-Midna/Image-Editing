...
def getGradients(img1, img2):
    """
    for the mixed gradients we want the biggest gradiant difference for an area 
    given both images.
    """
    
    imgConved = np.zeros_like(img1)
    for y in range(1, img1.shape[1]-1):
        for x in range(1, img1.shape[0]-1):
            left = (img1[x,y]-img1[x-1,y] if np.abs(img1[x,y]-img1[x-1,y]) > np.abs(img2[x,y]-img2[x-1,y]) else img2[x,y]-img2[x-1,y])
            right = (img1[x,y]-img1[x+1,y] if np.abs(img1[x,y]-img1[x+1,y]) > np.abs(img2[x,y]-img2[x+1,y]) else img2[x,y]-img2[x+1,y])
            top = (img1[x,y]-img1[x,y-1] if np.abs(img1[x,y]-img1[x,y-1]) > np.abs(img2[x,y]-img2[x,y-1]) else img2[x,y]-img2[x,y-1])
            bot = (img1[x,y]-img1[x,y+1] if np.abs(img1[x,y]-img1[x,y+1]) > np.abs(img2[x,y]-img2[x,y+1]) else img2[x,y]-img2[x,y+1])
            imgConved[x, y] = left+right+top+bot
            
    return imgConved

def getSumFQ(polygon, originalImg, mask, img2):
    """
    ...
    grads = getGradients(originalImg, img2)
    mp = mask-polygon
...
if __name__ == "__main__":
    ...
    plt.show()
    io.imsave("outputT2b.png", (output))
