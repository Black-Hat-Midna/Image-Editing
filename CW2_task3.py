...
def getGradients(img):
    """
    gets the gradients of the image using the gradient operator
    """
    
    imgConved = np.zeros_like(img)
    for y in range(1, img.shape[1]-1):
        for x in range(1, img.shape[0]-1):
            imgConved[x, y] = img[x, y]*4+(-img[x+1, y]-img[x-1,y]-img[x,y+1]-img[x,y-1])
...
if __name__ == "__main__":
    ...
    for i in range(0, len(imgs1)):
        ...
        B = getSumFQ(per1, imgs1[i], Fm, imgs2[i])
