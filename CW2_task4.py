...
def getGradients(img, canny):
    """
    gets the gradient for each pixel in the image provided it is in on the 
    boundery of an edge from a canny edge detector otherwise the gradient 
    for that direction is 0
    """
    
    #canny = feature.canny(img, sigma=1)
    imgConved = np.zeros_like(img)
    for y in range(1, img1.shape[1]-1):
        for x in range(1, img1.shape[0]-1):
            left = img[x,y]-img[x-1,y] if canny[x,y] or canny[x-1,y] else 0
            right = img[x,y]-img[x+1,y] if canny[x,y] or canny[x+1,y] else 0
            top = img[x,y]-img[x,y-1] if canny[x,y] or canny[x,y-1] else 0
            bot = img[x,y]-img[x,y+1] if canny[x,y] or canny[x,y+1] else 0
            imgConved[x, y] = left+right+top+bot
            
    return imgConved 

def getSumFQ(polygon, originalImg, mask, canny):
    """
    produces the values for B based on the sum of f star where the neighbouring
    points intersect with alpha omega and the gradients of the image at the given
    points -> provided by the mask
    """
    
    flatM = (mask-polygon).flatten()
    ind = np.where(flatM == 1)
    grads = getGradients(originalImg, canny)
    mp = mask-polygon
...
if __name__ == "__main__":
    ...
    img1 = cv2.resize(img1, (250, 400), interpolation = cv2.INTER_AREA)
    
    #img2 = cv2.imread('text1.jpg')
    ...
    canny = feature.canny(cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY), sigma=1)
    
    #plt.imshow(canny, cmap="gray")
    #plt.show()
    #img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2RGB).astype(np.float64)/255.
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB).astype(np.float64)/255.

    #process image for each colour channel
    #imgs2 = [img2[:, :, 0], img2[:, :, 1], img2[:, :, 2]]
    imgs1 = [img1[:, :, 0], img1[:, :, 1], img1[:, :, 2]]
    
    output = np.zeros(img1.shape)
    
    #get masks perimeters and image cutout
    Fm, fstar, per1 = getPolygon(imgs1[0], p)
    ...
    for i in range(0, len(imgs1)):
        ...
        B = getSumFQ(per1, imgs1[i], Fm, canny)

        X = solveLinearEqs(B, para)
        X = np.clip(X, 0, 1)
        output[:, :, i] = makeNewImage(Fm-per1, imgs1[i], X)

    plt.imshow(output)
    plt.show()
    io.imsave("outputT4.png", (output))
